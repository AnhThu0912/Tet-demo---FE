 ...
 */
export const createPayment = async (orderId: number): Promise<{ paymentId?: number }> => {
  ...
  const data = (json as ApiResponse<{ paymentId?: number }>)?.data ?? (json as any)?.data ?? json;
...
export const confirmPayment = async (orderId: number): Promise<{ success: boolean; message?: string }> => {
  const response = await fetch(`${API_BASE_URL}/orders/${orderId}/payments/confirm`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Cache-Control': 'no-cache',
